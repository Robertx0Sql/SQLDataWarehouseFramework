...
BEGIN 
	...
	exec [ADM].sp_generateMerge 
		...
		,@EmailProc = @EmailProc  -- EMail Stored PROC
	...
	SET @StepName = ' set @SQLHeader'; 
			...
			SET @SQLlogUpdate += @lf 
			SET @SQLlogUpdate += @lf + @tab + @tab + 'EXECUTE [ADM].[usp_UpdateETLLog] @LogId = @LogId, @DataCountInsert = @RowInsert, @DataCountUpdate = @RowUpdate, @DataCountDelete = @RowDelete, @DataCountTotalRows = @TotalRows,  @message = @ETLLOGUpdateMessage , @ErrorFlag = @ErrorFlag;'
			SET @SQLlogUpdate += @lf 
			SET @SQLlogUpdate += @lf + @tab + 'END TRY  '
			SET @SQLlogUpdate += @lf + @tab + 'BEGIN CATCH       -- Execute error retrieval routine.  '
			SET @SQLlogUpdate += @lf + @tab + @tab + 'EXECUTE [ADM].[usp_RethrowError] @LogId = @LogId ;'
			SET @SQLlogUpdate += @lf + @tab + 'END CATCH; '
			SET @SQLlogUpdate += @lf 
			SET @SQLlogUpdate += @lf + 'END'
			SET @SQLlogUpdate += @lf + 'GO'
		...
		+ ISNULL(@SQLLoadDate , '--??*ERROR*  @SQLLoadDate  ')  
